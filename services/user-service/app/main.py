...
app = FastAPI(
    title="User Service",
    description="Microservice pour la gestion des utilisateurs, rôles et authentification",
    version="1.0.0"
)
...
def require_role(required_role: str):
    def dependency(payload: dict = Depends(get_current_user_payload)) -> dict:
        ...
        if user_role != required_role:
            logger.warning(f"Accès refusé: Rôle {required_role} requis, Rôle {user_role} fourni")
            raise HTTPException(
...
def register_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    logger.info(f"Utilisateur créé avec succès: {new_user.username} (Rôle: {new_user.role})")
    return new_user
...
def login_user(login_data: UserLogin, db: Session = Depends(get_db)):
    ...
    if not user or not verify_password(login_data.password, user.hashed_password):
        ...
        logger.warning(f"Échec de la connexion pour l'utilisateur: {login_data.username}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    logger.info(f"Connexion réussie pour l'utilisateur: {user.username} (Rôle: {user.role})")
...
def get_all_users(db: Session = Depends(get_db), payload: dict = Depends(require_role("admin"))):
    logger.info(f"Administrateur {payload.get('sub')} demande la liste complète des utilisateurs")
